...
use crate::{ActionForKbgp, AppState, During, GAME_TITLE_FOR_MENU, GameOverReason};
...
fn draw_menu(mut egui_contexts: EguiContexts, mut frame_ui: ResMut<FrameUi>) -> Result<()> {
...
#[allow(dead_code)]
fn exit_button(mut frame_ui: ResMut<FrameUi>, mut exit: MessageWriter<bevy::app::AppExit>) {
    let Some(ui) = frame_ui.0.as_mut() else {
        return;
    };
    if ui
