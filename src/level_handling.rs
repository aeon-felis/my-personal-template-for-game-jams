...
fn read_last_finished_level(
    ...
    if let Some(completed_up_to_level) = pkv.and_then(|pkv| pkv.get::<String>(LEVEL_PKV_KEY).ok()) {
...
fn handle_level_completion(
    ...
    let finished_level_name = level_progress
        .current_level
        .take()
        .ok_or("current_level should be set when entering the LevelCompleted state")?;
